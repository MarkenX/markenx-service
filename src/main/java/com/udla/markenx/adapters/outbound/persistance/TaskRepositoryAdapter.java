...
public class TaskRepositoryAdapter implements TaskRepositoryPort {
    ...
    @Override
    public Page<Task> getStudentTasksByDueDate(Long studentId, LocalDate startDate, LocalDate endDate, int page, int size) {
        Pageable pageable = PageRequest.of(page, size);
    ...
    @Override
    public Page<Task> getStudentTasksByDueDateAndStatus(Long studentId, LocalDate startDate, LocalDate endDate, AssignmentStatus status, int page, int size) {
        Pageable pageable = PageRequest.of(page, size);
        return jpaRepository.findByStudentIdAndDueDateBetweenAndCurrentStatus(studentId, startDate, endDate, status, pageable)
            .map(TaskMapper::toDomain);
    }
}
